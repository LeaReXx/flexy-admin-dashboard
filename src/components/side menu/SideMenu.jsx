...
const SideMenu = ({ toggleMobileSideMenu, mobileSideMenu }) => {
  return (
    <div className="z-30 h-screen fixed">
      <div
        className={`w-screen h-screen fixed top-0 2xl:hidden bg-[#49494970] ${
          mobileSideMenu ? "opacity-100 visible" : "invisible opacity-0"
        }`}
        onClick={toggleMobileSideMenu}
      ></div>
      <article
        className={`bg-white shadow-md p-4 h-screen w-[250px] fixed 2xl:translate-x-0 2xl:static 2xl:visible top-0 left-0 duration-300 ease ${
          mobileSideMenu
